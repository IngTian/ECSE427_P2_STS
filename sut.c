...
void *C_EXEC() {
  ...
  thread_context *context_container =
  ...
  while (true) {
    ucontext_t *next_thread = pop_ready_queue();
    if (next_thread != NULL)
...
void *I_EXEC() {
  ...
  while (true) {
    ucontext_t *next_thread = pop_wait_queue();
    if (next_thread != NULL)
      swapcontext(current_context, next_thread);
    else
      usleep(100);
...
struct thread_context *get_parent_thread_context(pid_t thread_id) {
  for (int i = 0; i < g_number_of_threads; ++i)
    if (!g_parent_context_array[i] &&
        g_parent_context_array[i]->thread_id == thread_id)
...
void sut_init() {
  ...
  pthread_create(I_EXEC, NULL, i_exe, NULL);
  queue_insert_tail(&g_threads_queue, queue_entry(i_exe));

  for (int i = 0; i < NUM_OF_C_EXEC; i++) {
    pthread_t *c_exe = (pthread_t *)malloc(sizeof(pthread_t));
    pthread_create(C_EXEC, NULL, c_exe, NULL);
    queue_insert_tail(&g_threads_queue, queue_entry(c_exe));
  }
}
...
bool sut_create(sut_task_f fn) {
  ...
  append_to_ready_queue(queue_entry(new_context));
  return 1;
}
...
void sut_yield() {
  ...
  append_to_ready_queue(queue_entry(my_context));
  ucontext_t *parent_context = get_parent_thread_context(gettid());
...
int sut_open(char *dest) {
  ...
  ucontext_t *my_context = (ucontext_t *)malloc(sizeof(ucontext_t));
  getcontext(my_context);
  append_to_wait_queue(queue_entry(my_context));
  swapcontext(my_context, get_parent_thread_context(gettid()));
...
void sut_shutdown() {
  while (queue_peek_front(&g_threads_queue))
    pthread_kill(queue_pop_head(&g_threads_queue)->data);
  exit(0);
}